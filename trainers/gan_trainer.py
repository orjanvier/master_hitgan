...
class GANTrainer(base_trainer.BaseTrainer):
    ...
    with tf.GradientTape() as generator_tape, tf.GradientTape(
      ...
      if self.global_step%100==0:
        with self.summary_writer.as_default():
          tf.summary.image("real image", real_images, step=self.global_step)
          tf.summary.image("Generator output step:", fake_images, step=self.global_step)
      ...
      discriminator_real_loss, discriminator_fake_loss = losses.discriminator_loss(
          real_logits, fake_logits,
          loss_type=self._gan_loss_type) #+ grad_penalty + consistency_loss
      discriminator_total_loss = losses.discriminator_total_loss(real_logits,fake_logits,
          loss_type=self._gan_loss_type) + grad_penalty + consistency_loss
  ...
  def generate_one_image(self):
    latent_code = tf.random.normal(shape=(1,self._latent_dim))
    output = self.generator(latent_code, training=True)
